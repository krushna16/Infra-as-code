resource "aws_instance" "terraform-phpapp" {
  ...
  echo "\$conn = new mysqli('mydatabase.mypage.internal', 'root', 'secret', 'test');" >> /var/www/html/calldb.php
...
resource "aws_instance" "tearraform-database" {
  ami           = "${lookup(var.AmiLinux, var.region)}"
  instance_type = "t2.micro"
  ...
  mysql -u root -psecret -e "INSERT INTO mytable (mycol) values ('myfirstterraformpage') ;" test
HEREDOC
}
